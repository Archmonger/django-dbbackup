...
class PostgreSQLTestRunner:
    ...
    def setup_postgres(self):
        ...
        try:
            ...
            self.cleanup()
            raise RuntimeError(f"Failed to set up PostgreSQL: {e}")
    ...
    def _create_test_database(self):
        ...
        try:
            ...
            ], capture_output=True, use_sudo=True)
        except RuntimeError:
    ...
    def cleanup(self):
        ...
        if self.db_created:
            try:
                ...
                self._run_command([
                    "psql", "-c", drop_db_sql
                ], capture_output=True, check=False, use_sudo=True)  # Don't fail if database doesn't exist
                
                # Drop the test user
                drop_user_sql = f"DROP USER IF EXISTS {self.test_user};"
                self._run_command([
                    "psql", "-c", drop_user_sql
                ], capture_output=True, check=False, use_sudo=True)
            except Exception as e:
...
class PostgreSQLLiveTest:
    ...
    def _configure_django(self):
        ...
        backup_dir = os.path.join(self.postgres_runner.temp_dir, "backups")
    ...
    def _verify_test_data(self, expected_char_obj, expected_text_obj):
        ...
        if char_objs.count() != 1 or text_objs.count() != 1:
            raise AssertionError(f"Expected 1 of each model, found {char_objs.count()} CharModel and {text_objs.count()} TextModel")
            
        char_obj = char_objs.first()
        text_obj = text_objs.first()
        
        if char_obj.field != expected_char_obj.field:
            raise AssertionError(f"CharModel field mismatch: expected '{expected_char_obj.field}', got '{char_obj.field}'")
            
        if text_obj.field != expected_text_obj.field:
            raise AssertionError(f"TextModel field mismatch: expected '{expected_text_obj.field}', got '{text_obj.field}'")
...
def main():
    ...
    print("🐘 Starting PostgreSQL Live Tests for django-dbbackup")
    print("=" * 60)
    
    results = {}
    for connector in connectors_to_test:
        print(f"\n📋 Testing {connector}...")
        test = PostgreSQLLiveTest(connector, verbose=args.verbose)
        results[connector] = test.run_backup_restore_test()
